...
The **MCP server is already built** and browser-agnostic — it lives at `<FIREFOX_DIR>/server/`. This project is the Chrome extension only.
...
All logic to port lives in `<FIREFOX_DIR>/extension/`:
...
```bash
claude mcp add tethernet --scope user -- node <FIREFOX_DIR>/server/dist/index.js
```
...
3. Click **Load unpacked** → select `<EXTENSION_DIR>/`
...
- Firefox extension source: `<FIREFOX_DIR>/extension/`
- MCP server source: `<FIREFOX_DIR>/server/`
- Research findings: `<FIREFOX_DIR>/docs/chrome-extension-research.md`
- CLAUDE.md (project conventions): `<FIREFOX_DIR>/CLAUDE.md`
