...
A Chrome extension that connects Chrome to Claude Code via MCP. The MCP server already exists and does not change — only the browser extension changes. The server lives at `<FIREFOX_DIR>/server/`.
...
- Do NOT modify anything under `<FIREFOX_DIR>/server/`
